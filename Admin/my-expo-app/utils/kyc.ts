...
export interface KycFieldMatches {
...
export const resolveKycFieldMatches = (profile: any, aadhaarDataOverride?: any): KycFieldMatches => {
  const aadhaarData = aadhaarDataOverride || profile?.profileExtras?.aadhaarData;
  const hasAadhaarData = Boolean(aadhaarData);

  if (!hasAadhaarData) {
    return {
      hasAadhaarData: false,
      aadhaarName: "",
      aadhaarDob: "",
      aadhaarGender: "",
      nameMatch: false,
  ...
  const aadhaarDobParsed = parseDateOnly(aadhaarData?.dob);
  const dobMatch = Boolean(profileDob && aadhaarDobParsed && profileDob === aadhaarDobParsed);

  const profileGender = normalizeGender(profile?.sex);
  const aadhaarGenderNorm = normalizeGender(aadhaarData?.gender);
  const genderMatch = Boolean(profileGender && aadhaarGenderNorm && profileGender === aadhaarGenderNorm);
  ...
  return {
    ...
    dobMatch,
    genderMatch,
    allMatch: nameMatch && dobMatch && genderMatch,
  };
};
