...
interface Props {
  children: ReactNode;
  /** When map fails, show "Open in Maps" with these coords */
  latitude?: number;
  longitude?: number;
  onOpenMaps?: (lat: number, lng: number) => void;
  /** Optional label for the open button */
  label?: string;
}
 ...
 * isn't linked (e.g. dev build not rebuilt after adding react-native-maps).
 * Renders a fallback with "Open in Maps" so the app doesn't crash.
 */
export default class MapFallback extends Component<Props, State> {
  ...
  componentDidCatch() {
    // Error boundary: we show fallback UI
  }
