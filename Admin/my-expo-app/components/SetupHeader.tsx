...
export default function SetupHeader({ activeTab }: any) {
  ...
  const headerConfig: Record<string, { title: string; subtitle: string }> = {
    Property: {
      title: "Add New Property Details",
      subtitle: "Complete steps to generate your property ID and go live.",
    },
    Floors: {
      title: "Let's configure floors now...",
      subtitle: "Complete steps to generate your property ID and go live.",
    },
  };

  const headerTitle =
    headerConfig[activeTab]?.title || "Profile setup";

  const headerSubtitle =
    headerConfig[activeTab]?.subtitle ||
