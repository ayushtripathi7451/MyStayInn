...
export default function InfoCards() {
  ...
  const { stays, data: property, loading, error } = useCurrentStay();
  const list = stays.length > 0 ? stays : property ? [property] : [];
  const showLoading = list.length === 0 && !error && loading;
  const cardWidth = Math.min(width - 48, width * 0.88);
      ...
      {showLoading ? (
        ...
        >
          {list.map((property, idx) => {
            const cardKey = `${String(property.bookingId ?? property.id)}-${idx}`;
            ...
            const moveIn =
              property?.checkInDate
                ...
                : "—";
            return (
              <View key={`stay-card-${cardKey}`} style={{ width: cardWidth }}>
                <TouchableOpacity
                  activeOpacity={0.88}
                    ...
                    <View className="flex-row flex-wrap gap-4 mb-4">
                      ...
                      <View className="flex-row items-center">
                        ...
                        <View className="ml-2">
                          ...
                          <Text className="text-sm font-semibold text-slate-800">{moveIn}</Text>
                        </View>
                      </View>
                    </View>
